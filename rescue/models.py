...
class Group(models.Model):
    ...
    def results_round_2(self):
        return self.performances.filter(round_number=2).order_by('points', 'time').order_by('time').reverse()

    def results_round_3(self):
        return self.performances.filter(round_number=3).order_by('points', 'time').order_by('time').reverse()
